...
<div class="recipe" style="width=400px; float:left">
	<div class="row" id="test" align="center" style="padding-bottom:10px; margin-left: 230px; width: 100%; background-color: #f8f8f8; margin-top: -1000px">
			<div class="typename"  style="padding-left:35px ; padding-top: 10px; margin-bottom: -20px">
			<p><b><h2 class="titles"> 
			<?php
...
//echo $rid;
		$sql="SELECT photo, photoid
			FROM recipe_photo
			WHERE recipe_photo.rid=$rid LIMIT 5 ";
			$result_photo = mysqli_query($conn,$sql);
			
			if (mysqli_num_rows($result_photo) > 0) {
				
				
	while($photo = mysqli_fetch_assoc($result_photo)) {
		//$rid=$row_rv["rid"];
		//echo $photo["photo"];
		if($photo["photo"]!=null)
		echo  '<img src="data:image/jpeg;base64,' . base64_encode($photo["photo"]) . '" width="200" height="200">';
		
		else echo  '<img src="images/nophoto.png" width="200" height="200">';
		?>
	...
	<div class="row" align="center" id="test" style="padding-bottom:10px; margin-left: 450px; width: 100%; background-color: #f8f8f8; margin-top: -30px">
...
<div class="col-sm-offset-1 col-sm-6" style="margin-top: 10px; margin-bottom: -20px">
<div class="text">
<h4 align="center"><u> REVIEWS: </u> </h4>
<fieldset>
...
<div class="tags" style="margin:left:400px; width:400px; margin-top: -1000px; float:right; padding-right:50px">
<b>
<p align="center">
<h3 style="color:grey">
MORE RECIPES BASED ON:
</h3> 
<br>
<p align="center"  style="font-size:24px">
<?php
		$sqltags = "SELECT recipe_has_tag.tid, tagname from recipe_has_tag join tags on recipe_has_tag.tid=tags.tid where recipe_has_tag.rid=$rid"; 
		//echo $sqltags;
		//exit();
		$resulttags = mysqli_query($conn, $sqltags);
		if (mysqli_num_rows($resulttags) > 0) {
		while($rowtags = mysqli_fetch_assoc($resulttags)) {
		//echo $rowtags["tagname"]."<br>";
		?><div class="links" style="width:80%; height: 50px; background-color:#A52A2A; margin-top:-5px; border-radius:5px">
			<a href="relatedRecipesByTags.php?tid=<?php echo $rowtags["tid"]?>&rid=<?php echo $rid?>&tagname=<?php echo $rowtags["tagname"] ?>" style="text-decoration: none; color:white"><h4 align="center" style="padding:10px"> <?php echo $rowtags["tagname"]?></h4></a>
			</div>
			<br>
			<?php
		}
