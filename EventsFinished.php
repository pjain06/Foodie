...
<h3 class="page-header"> Events which finished recently</h3>
<?php include_once("DBconnect.php");

$sql = "select eid,gid,eventName, eDescription, locationCity,startTime,endTime, gname from event natural join group_members natural join cooking_group where  endTime<now() and uid='$uid';";
if ($result = $conn->query($sql))
{
	echo "<table  class='table table-bordered '>\n";
	if ($result->num_rows > 0) {
    // output data of each row with headers
		echo "\t<th>Event Name</th><th>Event Description</th><th>Event Location </th><th>Event start date</th><th>Event end date</th><th>GroupName</th><th>Report</th>\n";
		while($row = $result->fetch_assoc()) {
			...
			$query="select * from event_report where eid=$row[eid]";
			$newresult = $conn->query($query);
			if ($newresult->num_rows >0){
			echo "\t<td><a href=viewReport.php?eid=$row[eid]>View Report</a></td>";}
			else{
			echo "\t<td>No reports available</td></tr>\n";
			}
	}
		echo "</table>\n";
	}
